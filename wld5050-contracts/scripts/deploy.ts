...
async function main() {
  ...
  console.log("\n── Next steps ──────────────────────────────────────")
  console.log(`1. Root .env.local + Vercel:`)
  console.log(`   NEXT_PUBLIC_WLD5050_CONTRACT=${address}`)
  console.log(`2. World ID Developer Portal (https://developer.worldcoin.org):`)
  console.log(`   App ID must match NEXT_PUBLIC_WLD_APP_ID (contract APP_ID constant)`)
  console.log(`   Actions: create-raffle, enter-raffle-{id}`)
  console.log(`   RP ID → WORLD_RP_ID in Vercel (server only, not in contract)`)
  console.log(`   Signing key → WORLD_RP_SIGNING_KEY in Vercel (server only)`)
  console.log(`3. Update workflow/config.production.json:`)
  console.log(`   "wld5050Contract": "${address}"`)
  console.log(`4. Deploy CRE workflow (30s cron):`)
  console.log(`   cd workflow && bun install && cd ..`)
  console.log(`   cre workflow deploy workflow --target production-settings`)
  console.log(`   cre workflow activate workflow --target production-settings`)
  console.log(`5. Full checklist: see DEPLOY.md in repo root`)
  console.log("────────────────────────────────────────────────────")
}
